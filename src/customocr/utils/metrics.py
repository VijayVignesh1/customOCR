...
def compute_cer(gt_texts, pred_texts):
    """Compute Character Error Rate (CER) for a batch of strings. CER = (edit
    distance) / (total characters in GT)
...
def compute_wer(gt_texts, pred_texts):
    """Compute Word Error Rate (WER) for a batch of strings. WER = (edit
    distance between word sequences) / (total words in GT)
